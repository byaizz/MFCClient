...
//#define GRIDCONTROL_NO_TITLETIPS   // Do not use titletips for cells 
                                     // with large data
...
//#define GRIDCONTROL_NO_TITLETIPS    // Do not use titletips LUO..
...
// Modified by PRAS 定义表格背景色
#define COLOR_FUCHSIN_EVEN     RGB(238, 229, 222)
#define COLOR_FUCHSIN_ODD      RGB(255, 255, 255)
#define COLOR_FUCHSIN_FIXED    RGB(232, 232, 232)

#define COLOR_BLUE_EVEN        RGB(226, 234, 248)
#define COLOR_BLUE_ODD         RGB(255, 255, 255)
#define COLOR_BLUE_FIXED       RGB(231, 224, 232)//RGB(143, 166, 143)

// Modified by PRAS 设置表格背景色风格
#define GRID_COMMON             0
#define GRID_FUCHSIN            1
#define GRID_BLUE               2
...
// Cell Edit Type // Modified by PRAS
#define GVET_TEXT               0
#define GVET_INT                1
#define GVET_DOUBLE             2
#define GVET_LIST               3
...
#define GVN_FIXEDROWCLICK       HDN_ITEMCLICK    // Modified by PRAS,Listmode



// Each cell contains one of these. Fields "row" and "column" are not stored
// since we will usually have acces to them in other ways, and they are an 
...
public:
    CGridCell() 
    {
        state = 0;
        nFormat = 0;
        nEditType = GVET_TEXT;  // Modified by PRAS 默认编辑方法为字符串
        szText.Empty();
        iImage = -1;
        crBkClr = CLR_DEFAULT;
        crFgClr = CLR_DEFAULT;
    ...
    UINT     nEditType;   // Modified by PRAS 标识编辑框的类型
...
// situations the usual DDX_Control does not result in 
// CGridCtrl::SubclassWindow or CGridCtrl::PreSubclassWindow being called. 
// Using this version calls  CGridCtrl::SubclassWindow directly - ensuring 
// that cell metrics are set properly
class CGridCtrl;
...
// Construction
public:
    CGridCtrl(int nRows = 0, int nCols = 0, 
...
public:
    // Modified by PRAS
    void SetGridStyle(int iStyle)               { m_iGridStyle = iStyle; }
    ...
    // 设置表格是否可以排序
    void     SetItemsSort(BOOL bSort)           
                                { m_bSortItems = bSort;             }
    // 设置表格模式，m_bListMode为全行选中
    void     SetGridMode(BOOL bMode)           
                                { m_bListMode = bMode;             }
    ...
    BOOL   SetRowBkColour(int nRow, COLORREF cr = CLR_DEFAULT);
...
public:
    ...
    void AutoSize();
    void _Invalidate();  // MOD BY LUO..添加更新滚动条功能
    void ExpandColumnsToFit();
    void ExpandRowsToFit();
    void ExpandToFit();

    // Modified by PRAS 清空CTRL键保留数据
    void ClearPrevSelectedCellMap()   { m_PrevSelectedCellMap.RemoveAll(); }

    // Modified by PRAS 设置光标位置
    void SetFocusedCell(int nRow, int nCol) { SetFocusCell(nRow, nCol);}
    ...
    // SetRedraw stops/starts redraws on things like changing the # 
    // rows/columns and autosizing, but not for user-intervention 
    // such as resizes
    ...
    void Print(); 
    //void PrintPreview();// Kah Hoe added

    void PrintPreview(const int iGridID = 0);// Modified by PRAS
    
    CSingleDocTemplate* m_pTemplate;// Kah Hoe added
    BOOL m_bPrintPreview;// Kah Hoe added
#endif
    ...
    void SetCellEditType(int nRow, int nCol, UINT nEditType); 
...
public:
    ...
    static int CALLBACK pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2, 
                                        LPARAM lParamSort);// Modified by PRAS
    static int CALLBACK pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, 
                                        LPARAM lParamSort);// Modified by PRAS
    void SetCompareFunction(PFNLVCOMPARE pfnCompare);
    virtual ~CGridCtrl();

    void ScrollToRow(int nRowIndex);
    void ResetScrollBars();
    //获取鼠标所在的行列
    void  GetCellPoint(CCellID& cell)  { cell = m_MouseCellPoint; return ; }
    void  GetMousePoint(CPoint point)  { point = m_Point; return ; }
...
protected:
    ...
    int  SetMouseMode(int nMode) { int nOldMode = m_MouseMode; 
                                   m_MouseMode = nMode; return nOldMode; }
...
protected:
...
#ifndef GRIDCONTROL_NO_DRAGDROP
    // Drag n' drop， no longer necessary
    ...
    // Modified by PRAS 增加nEditType参数，判断edit的类型
    virtual void  CreateInPlaceEditControl(CRect& rect, DWORD dwStyle, 
                                           UINT nID, int nRow, int nCol,
                                           LPCTSTR szText, int nChar, 
                                           UINT nEditType);
   

    // Modified by PRAS 创建List
    virtual void  CreateInPlaceListControl(CRect& rect, DWORD dwStyle, 
                                           UINT nID, int nRow, int nCol,
                                           LPCTSTR szText, int nChar);  

    // Drawing
    virtual void  OnDraw(CDC* pDC);
    virtual BOOL  DrawFixedCell(CDC* pDC, int nRow, int nCol, 
                                CRect rect, BOOL bEraseBk=FALSE);
    virtual BOOL  DrawCell(CDC* pDC, int nRow, int nCol, CRect rect, 
                           BOOL bEraseBk=FALSE);
...
protected:
    ...
    BOOL        m_bTitleTips;
    BOOL        m_bSortItems;   // Modified by PRAS 设置表格是否可排序

    int         m_iGridStyle;    // Modified by PRAS 设置表格的风格
    // Cell size details
    ...
    CFont       m_PrinterTitleFont; // Modified by PRAS 标题的字体
    CFont       m_PrinterHeaderFont; // Modified by PRAS 页眉的字体
    ...
    int         m_MouseMode;
    CPoint      m_LeftClickDownPoint, m_LastMousePoint, m_Point;
    CCellID     m_LeftClickDownCell, m_SelectionStartCell, m_MouseCellPoint;
    ...
    CSize       m_CharSize;
    CSize       m_GapCharSize;          // Modified by PRAS 空白间隙的大小
    CSize       m_TitleCharSize;        // Modified by PRAS 标题字体的大小
    CSize       m_HeaderCharSize;       // Modified by PRAS 页眉字体的大小
    int         m_nPageHeight;
    ...
    int         m_bAscending;
    int         m_SortColumn;
    // Modified by PRAS 函数指针，指向表格内容比较函数
    PFNLVCOMPARE m_pfnCompare;

protected:
    void CreatePrinterFont(CDC *pDC);   // Modified by PRAS
...
BOOL GetPrinterDevice(LPTSTR pszPrinterName, HGLOBAL* phDevNames, 
                      HGLOBAL* phDevMode);
