...
// Added by Kah Hoe
#include "ViewPrintPreview.h" // View class for preview view
// Added by Kah Hoe (To recognize resource ID IDR_MENU)
#include "resource.h"
...
void AFXAPI DDX_GridControl(CDataExchange* pDX, int nIDC, CGridCtrl& rControl)
{
    if (rControl.GetSafeHwnd() == NULL)    // not subclassed yet
    {
        ...
        else
        {
            ...
            if (pDX->m_pDlgWnd->GetSafeHwnd() 
                != ::GetParent(rControl.GetSafeHwnd()))
...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_iGridStyle          = GRID_COMMON;     // Modified by PRAS
    m_nRows              = 0;
    m_nCols              = 0;
    m_nFixedRows         = 0;
    m_nFixedCols         = 0;
    ...
    //m_bEnableSelection   = FALSE/*TRUE*/;   // Modify by TP 取消整行选中 20140619
    m_bEnableSelection   = TRUE;   // 整行选中
    ...
    m_SortColumn         = -1;
    m_pfnCompare         = NULL;      // Modified by PRAS 比较函数指针默认NULL
    ...
    m_nResizeCaptureRange = 3;        // When resizing columns/row, the cursor
                                      // has to be 
                                      // within +/-3 pixels of the dividing  
                                      // line for resizing to be possible
    m_pImageList         = NULL;      
    ...
    lstrcpy(ncm.lfMessageFont.lfFaceName, _T("宋体")); // Modied by PRAS
    ncm.lfMessageFont.lfHeight = 14;                   // Modied by PRAS
    ncm.lfMessageFont.lfWeight = FW_BOLD;              // Modied by PRAS
    memcpy(&m_Logfont, &(ncm.lfMessageFont), sizeof(LOGFONT));
#else
    LOGFONT lf;
    GetObject(GetStockObject(SYSTEM_FONT), sizeof(LOGFONT), &lf);
    lstrcpy(lf.lfFaceName, _T("宋体"));                // Modied by PRAS
    lf.lfHeight = 14;                                  // Modied by PRAS
...
CGridCtrl::~CGridCtrl()
{
    ...
    m_TitleTip.DestroyWindow();                  // MOD BY PRAS
#endif
                       ...
                       UINT nID, DWORD dwStyle)
{
    ...
    // created using a dialog template then the number of rows and columns will
    // be 0 (which means that the code below will not be needed - which is 
    // lucky 'cause it ain't gonna get called in a 
    // dialog-template-type-situation.
...
BEGIN_MESSAGE_MAP(CGridCtrl, CWnd)
    ...
    ON_WM_LBUTTONUP()
    ON_WM_LBUTTONDOWN()
    ON_WM_RBUTTONUP()       // Modified by PRAS 添加右键up消息响应
    ON_WM_RBUTTONDOWN()     // Modified by PRAS 添加右键down消息响应
    ON_WM_LBUTTONDBLCLK()   // Modified by PRAS 添加左键双击消息响应
    ON_WM_MOUSEMOVE()
    ON_WM_TIMER()
    ON_WM_GETDLGCODE()
    ON_WM_KEYDOWN()
...
// memDC's selected bitmap with colour, then all sorts of vis problems 
// would occur
...
//  if (::IsWindow(GetSafeHwnd()) && GetFocus()->GetSafeHwnd() 
        //!= GetSafeHwnd())
...
void CGridCtrl::OnSysColorChange() 
{
    ...
    // Still using system colours
    if (GetTextColor() == m_crWindowText)
        // set to new system colour
        ...
        case VK_TAB:    
            if (IsSHIFTpressed())
            {
                ...
                else if (next.col == m_nFixedCols && next.row > m_nFixedRows) 
                {
                    ...
                    // Modified by PRAS 不让选中最后一列
                    // LUO 20140521 去除最后一行空白
...
{    
    ...
    switch (pgvItem->lParam) 
    {
        ...
        case VK_END:    
            ...
            OnEditCell(m_idCurrentCell.row, m_idCurrentCell.col, 
                        (UINT)pgvItem->lParam);
    }
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    for (row = 0; row < m_nFixedRows; row++)
    {
        ...
        {                                       
            ...
            // Modify by PRAS,Set last fixed column width.
            //if (col == GetColumnCount()-1)
            //{
            //    CRect myrect;
            //    GetClientRect(myrect);
            //    // Modified by PRAS,Save change to cell.
            //    SetColumnWidth(col,myrect.right-rect.right); 
            //    rect.right = myrect.right;
            //}
            //else  // Modified by PRAS,Not last column
            //{
            //    rect.right = rect.left + GetColumnWidth(col)-1;
            //}
            // LUO 20140521 去除最后一行空白
    ...
    if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_HORZ) 
    {
        ...
        // Modified by PRAS,Get client rect.
        // LUO 20140521 去除最后一行空白
        /*CRect myrect;
        GetClientRect(myrect);*/
        for (row = minVisibleRow; row <= maxVisibleRow; row++) {
            ...
            // Modified by PRAS,Line to client right.
            // LUO 20140521 去除最后一行空白
            //pDC->LineTo(myrect.right,  y-1);
...
void CGridCtrl::ScrollToRow(int nRowIndex)
{
    ...
    int nRowHeight = GetRowHeight(nRowIndex);
    int nRowCount = GetRowCount();
    int nOrgPos = GetScrollPos32(SB_VERT);
    ...
    float fPos = (float)(nRowIndex-2) / (float)nRowCount;
    ...
    SetScrollPos32(SB_VERT, (int)fPos, TRUE);
    ...
    rect.top = GetFixedRowHeight();
    //rect.top = GetFixedRowHeight() + yScroll;
    ScrollWindow(0, (int)((float)nOrgPos - fPos), &rect);
    //ScrollWindow(0, (int)(-nMaxPos), &rect);
    //rect.top = rect.bottom - yScroll;
    ...
    int nRow = nRowCount / 2;

    if ( nRowIndex >= nRowCount /* || nRowIndex < nRow*/)
    {
        return;
    }
                                 ...
                                 int nMaxCol, BOOL bForceRepaint /* = FALSE */)
{
    ...
    // if any previous selected cells are to be retained 
    // (eg Ctrl is being held down)
    // then copy them to the newly created list, and mark all these cells as
    // selected
    if (!GetSingleRowSelection())
    for (POSITION pos = m_PrevSelectedCellMap.GetStartPosition(); 
         pos != NULL; /* nothing */)
    {
        DWORD key;
        CCellID cell;
    ...
    // Now select all cells in the cell range specified. If the cell has 
    // already been marked as selected (above) then ignore it.
         ...
         nMinRow <= nMaxRow && nMinCol <= nMaxCol)
    {
        for (int row = nMinRow; row <= nMaxRow; row++)
            for (int col = nMinCol; col <= nMaxCol; col++) 
            {
                ...
                // Set state as Selected. This will add the cell 
                // to m_SelectedCells[]
...
COleDataSource* CGridCtrl::CopyTextFromGrid()
{
    ...
    // Modified by PRAS 修改在listmode下的复制方式，
    // 如果是点击左标题选中一行则按行拷贝，否则拷贝单元格
    CCellRange Selection;
    CCellID LastCellID = GetCellFromPt(m_LastMousePoint);
    if (m_bListMode && LastCellID.col >= GetFixedColumnCount())
    {
        CCellID FocusCellID = GetFocusCell();
        Selection.Set(FocusCellID.row, FocusCellID.col, 
                      FocusCellID.row, FocusCellID.col);
    }
    else
    {
    ...
    //(REMEBER: CF_TEXT is ANSI, not UNICODE, so we need to convert)
...
{
    // Modified by PRAS 将ReadOnly的判断放在后面每个单元格而不是这里
    ...
    // to hold this.
    PTCHAR szBuffer = new TCHAR[::GlobalSize(hmem)];
    if (!szBuffer)
        return FALSE;
        ...
        while (pszCellText != NULL)
        {
            ...
            if (IsValid(TargetCell))
            {
                ...
                SetItemState(TargetCell.row, TargetCell.col,
                             GetItemState(TargetCell.row, TargetCell.col) 
                             & ~GVIS_SELECTED);
            }
                                 ...
                                 CPoint point)
{
    ...
    // If not valid, set the previously drop-highlighted cell 
    // as no longer drop-highlighted
    if (!IsValid(cell))
    {
        OnDragLeave();
    ...
    {
        // Set the previously drop-highlighted cell 
        // as no longer drop-highlighted
        if (IsValid(m_LastDragOverCell)) {
            UINT nState = GetItemState(m_LastDragOverCell.row, 
                                       m_LastDragOverCell.col);
    ...
    // Return an appropraite value of DROPEFFECT so mouse cursor 
    // is set properly
                                  ...
                                  DWORD dwKeyState, CPoint point)
{
    ...
    // Return an appropraite value of DROPEFFECT 
    // so mouse cursor is set properly
    if (dwKeyState & MK_CONTROL)
        return DROPEFFECT_COPY;
    else
        return DROPEFFECT_MOVE;
...
void CGridCtrl::OnEditPaste()
{
    ...
    if (m_bListMode)  // Modified by PRAS listmode下粘贴完重新将整行选中 
    {
        m_SelectionStartCell.row = cell.row;
        SelectRows(cell);
    }
}
#endif
...
CCellID CGridCtrl::GetCellFromPt(CPoint point, 
                                 BOOL bAllowFixedCellCheck /*=TRUE*/) const
{  
    CCellID idTopLeft = GetTopleftNonFixedCell();
    ...
    // Modified by PRAS 最后一列不让鼠标选中
    // LUO 20140521 去除最后一行空白
    /*if (cellID.col == GetColumnCount() -1)
    {
        cellID.col = -1;
        cellID.row = -1;
    }*/
...
CCellRange CGridCtrl::GetVisibleNonFixedCellRange(LPRECT pRect /*=NULL*/) const
{
    ...
    return CCellRange(idTopLeft.row, idTopLeft.col, 
...
void CGridCtrl::ResetScrollBars()
{
    ...
    // Modified by PRAS 可视区域没有非Fixed区域，只要有表格，都更新滚动条
    //CCellRange visibleCells = GetUnobstructedNonFixedCellRange();
    //if (!IsValid(visibleCells)) return;
    if (m_nRows < 0 || m_nCols < 0) return;     // Modified by PRAS 添加

    /*
     * 设定滚动条位置，将代码移到下方，设定边界后再设定位置
     * LUO..20110628
     */
    /*SCROLLINFO si;
    si.cbSize = sizeof(SCROLLINFO);
                                 ...
                                 LPRECT lpRect) const
{
    ...
    if (!GetCellOrigin(cellRange.GetMinRow(),
        cellRange.GetMinCol(), &MinOrigin)) 
        return FALSE;
    if (!GetCellOrigin(cellRange.GetMaxRow(), 
        cellRange.GetMaxCol(), &MaxOrigin)) 
        return FALSE;
...
LRESULT CGridCtrl::OnSetFont(WPARAM hFont, LPARAM /*lParam */)
{
    ...
    if (pDC) 
    {
        ...
        /*
         * MOD BY LUO..  减小数据与边框的间隔
         */
        //m_nMargin = int(pDC->GetTextExtent(_T(" "),1).cx);
        m_nMargin = int(pDC->GetTextExtent(_T(" "),1).cx * 3 / 4);
...
BOOL CGridCtrl::SetColumnCount(int nCols)
{
    ...
    // If we are about to lose columns, then we need to delete  
    // the GridCell objects within each column
    if (addedCols < 0) {
...
// Insert a column at a given position, or add to end of columns 
// (if nColumn = -1)
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    {
        m_arColWidths.InsertAt(nColumn, (int)0, (int)1);
        for (int row = 0; row < m_nRows; row++) 
        {
            GRID_ROW* pRow = m_RowData[row];
            if (!pRow) return -1;
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    //if (nRow >= 0 && nRow >= GetRowCount())   // MOD BY PRAS
    ...
    {
        m_arRowHeights.InsertAt(nRow, (int)0, (int)1);
    ...
    // MOD BY LUO..插入时不自动执行，插入完成后由外部显示调用
    //ResetScrollBars();
...
BOOL CGridCtrl::DeleteRow(int nRow)
{
    ...
    // Modified by PRAS 删除一行时，首先清除"CTRL"键保留的数据；
    // 所有选中的数据不是全部清除，而是删除一行清除一行，实现多行删除。
    m_PrevSelectedCellMap.RemoveAll();
    //ResetSelectedRange();
...
BOOL CGridCtrl::DeleteAllItems()
{
    ...
    for (int row = 0; row < m_nRows; row++)
    {
        ...
        for (int col = 0; col < m_nCols; col++)
        {
            CGridCell* pCell = pRow->GetAt(col);
            if (pCell) {
                // TODO - this is a bit of a performance hit.
                EmptyCell(pCell, row, col);
...
// recursive sort implementation
// Modified by PRAS 该函数中将GetItemData用GetCell代替
BOOL CGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, 
                          ...
                          int low, int high)
{
    ...
    LPARAM midItem = (LPARAM)GetCell/*GetItemData*/( (lo+hi)/2, nCol );
    ...
    while( lo <= hi )
    {
        ...
        if( bAscending )
            while (lo < high  
                && pfnCompare((LPARAM)GetCell/*GetItemData*/(lo, nCol), 
                midItem, data) < 0)
                ++lo;
        else
            while (lo < high 
        ...
        if( bAscending )
            while (hi > low 
                && pfnCompare((LPARAM)GetCell/*GetItemData*/(hi, nCol),
                midItem, data) > 0)
                --hi;
        else
            while (hi > low 
        ...
        {
            // swap only if the items are not equal
            if (pfnCompare((LPARAM)GetCell/*GetItemData*/(lo, nCol), 
                (LPARAM)GetCell/*GetItemData*/(hi, nCol), data) != 0)
            {
                for (int col = 0; col < GetColumnCount(); col++)
                {
                    CGridCell *pCell = GetCell(lo, col);
...
BOOL CGridCtrl::SetItem(const GV_ITEM* pItem)
{
    ...
    if (pItem->mask & GVIF_FONT)   memcpy(&(pCell->lfFont), 
        &(pItem->lfFont), sizeof(LOGFONT));
...
BOOL CGridCtrl::GetItem(GV_ITEM* pItem)
{
    ...
    if (pItem->mask & GVIF_TEXT)   pItem->szText  
                                    = GetItemText(pItem->row, pItem->col);
    ...
    if (pItem->mask & GVIF_FONT)   memcpy(&(pItem->lfFont), 
                                       &(pCell->lfFont), sizeof(LOGFONT));
...
BOOL CGridCtrl::SetRowBkColour(int nRow,
                                COLORREF cr /* = CLR_DEFAULT */)
{
    for (int iCol = 0; iCol < m_nCols; iCol++)
    {
        CGridCell* pCell = GetCell(nRow, iCol);
        ASSERT(pCell);
        if (!pCell) 
            return FALSE;
...
BOOL CGridCtrl::AutoSizeColumn(int nCol)
{
    ...
    // 如果是多行显示，则本单元格列宽不变 LUO..20110624
    if (GetRowCount() > 1)
    {
        if (GetItemFormat(1, nCol) & DT_EDITCONTROL)
        {
            return TRUE;
        }
    }
...
BOOL CGridCtrl::AutoSizeRow(int nRow)
{
    ...
    {  
         ...
         * 根据文本长度确定行高 LUO..
         */
        if (GetItemFormat(nRow, nCol) & DT_EDITCONTROL)
        {
            nWidth = m_arColWidths[nCol];
            if (size.cx > nWidth)
            {
                size.cy = (size.cy - 1 * m_nMargin) * ((int)(size.cx / nWidth) + 1)
                        + 2 * m_nMargin;
            }
        }
...
// MOD BY LUO..添加更新滚动条功能
void CGridCtrl::_Invalidate()
{
    if (m_bAllowDraw) {
        ResetScrollBars();
        Invalidate();
...
void CGridCtrl::ExpandToFit()
{
    ...
    ExpandColumnsToFit();   // Just in case the first adjustment was 
                            ...
                            int nRow /*=-1*/, int nCol /*=-1*/)
{
    ...
    if (!m_bModified)
    {
        for (int row = 0; row < GetRowCount(); row++)
            for (int col = 0; col < GetColumnCount(); col++) 
                SetItemState(row, col, 
                    GetItemState(row, col) & ~GVIS_MODIFIED);
    }
}
...
// Note - if this doesn't work for win95 then 
// use OnRegisteredMouseWheel instead
#if !defined(_WIN32_WCE) && (_MFC_VER >= 0x0421)
...
void CGridCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    //gao 20170528 start
    m_MouseCellPoint.col = -1;
    m_MouseCellPoint.row = -1;  //gao 20170528 end
    ...
    // If the left mouse button is up, then test to see 
    // if row/column sizing is imminent
        ...
        || (m_MouseMode == MOUSE_NOTHING && (nFlags & MK_LBUTTON)))
    {
        ...
        if (m_MouseMode == MOUSE_NOTHING && m_bTitleTips)
        {
            ...
            //gao 20170528 start
            //if (idCurrentCell.col >=0 && idCurrentCell.row >= 0)
            //{
            //    m_MouseCellPoint = idCurrentCell;
            ...
            m_Point = point;//gao 20170528 end
            ...
            if (GetTextRect(idCurrentCell.row, idCurrentCell.col, TextRect) &&
                ...
                && ((GetItemFormat(idCurrentCell.row,idCurrentCell.col) & DT_EDITCONTROL)
                < DT_EDITCONTROL))  // LUO..20110622 多行显示数据不显示提示框
                m_TitleTip.Show(TextRect, 
                                GetItemText(idCurrentCell.row, 
                                            idCurrentCell.col), 
                                0,
                                CellRect,
                                GetItemFont(idCurrentCell.row, 
                                            idCurrentCell.col));
        }
#endif
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    m_SelectionStartCell = 
            (nFlags & MK_SHIFT)? m_idCurrentCell : m_LeftClickDownCell;
    ...
    if (m_bAllowDragAndDrop && hOldFocusWnd == GetSafeHwnd() && 
        GetItemState(m_LeftClickDownCell.row, m_LeftClickDownCell.col) 
        & GVNI_SELECTED)
    {
    ...
    {    
        ...
        if (nFlags & MK_CONTROL) {
            for (POSITION pos = m_SelectedCellMap.GetStartPosition(); 
                 pos != NULL; )
            {
                DWORD key;
                CCellID cell;
    ...
    // Modified by PRAS 向父窗口发送按键消息
    CCellID cell = GetCellFromPt(point);
    SendMessageToParent(cell.row, cell.col, WM_LBUTTONDOWN);
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    // m_MouseMode == MOUSE_PREPARE_EDIT only if user clicked down 
    // on current cell
...
#ifndef GRIDCONTROL_NO_DRAGDROP
    // m_MouseMode == MOUSE_PREPARE_DRAG only if user clicked down 
    // on a selected cell
    // and then didn't move mouse before clicking up (releasing button)
    ...
    else if (m_MouseMode == MOUSE_SIZING_COL)
    {
        CRect rect;
        GetClientRect(rect);
        CRect invertedRect(m_LastMousePoint.x, rect.top, 
                           m_LastMousePoint.x + 2, rect.bottom);
    ...
    else if (m_MouseMode == MOUSE_SIZING_ROW)
    {
        CRect rect;
        GetClientRect(rect);
        CRect invertedRect(rect.left, m_LastMousePoint.y, 
                            rect.right, m_LastMousePoint.y + 2);
    ...
    // 发送左键双击消息到父窗口
    //CCellID cell = GetCellFromPt(point);
    //SendMessageToParent(cell.row, cell.col, BN_CLICKED);
}

#if !defined(_WIN32_WCE_NO_PRINTING) && !defined(GRIDCONTROL_NO_PRINTING)

// Modified by PRAS 添加右键down消息响应
void CGridCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
    ...
    {
        CCellID CellRef;
        CCellID oldCell = GetFocusCell();
        DWORD key = MAKELONG(cell.row, cell.col);

        // 右键点击时，先设定鼠标焦点为表格 LUO..20110704
        SetFocus();

        // 根据右键点击的情况决定是否更改selection
        if (cell != oldCell 
            && !m_SelectedCellMap.Lookup(key, (CCellID&)CellRef))
        {
            ...
            {
                key = MAKELONG(cell.row, GetFixedColumnCount());
                if (!m_SelectedCellMap.Lookup(key, (CCellID&)CellRef))
                {
                    m_PrevSelectedCellMap.RemoveAll();
                    SetSelectedRange(cell.row, GetFixedColumnCount(),
                                     cell.row, GetColumnCount()-1);
                }
                CellRef.row = cell.row;
                CellRef.col = GetFixedColumnCount();
                SetFocusCell(CellRef);
            }
            else if(!m_bListMode)
            {
                m_PrevSelectedCellMap.RemoveAll();
                SetSelectedRange(cell.row, cell.col,
                                 cell.row, cell.col);
            }
    ...
    SendMessageToParent(cell.row, cell.col, WM_RBUTTONDOWN);
    CWnd::OnRButtonDown(nFlags, point);
    return;
}

// Modified by PRAS 添加右键up消息响应
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
    CCellID cell = GetCellFromPt(point);
    SendMessageToParent(cell.row, cell.col, WM_RBUTTONUP); 
...
void CGridCtrl::Print() 
{
 ...
 //  Info.m_rectDraw.SetRect(0,0, dc.GetDeviceCaps(VERTRES), 
...
void CGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    // (will be a screen DC for print previewing)
    ...
    // Get the average character width (in GridCtrl units) 
    // and hence the margins
    m_CharSize = 
        pDC->GetTextExtent(
...
#ifdef _WIN32_WCE
    m_LogicalPageSize.cy = (m_LogicalPageSize.cx * m_PaperSize.cy) 
                            / m_PaperSize.cx;
#else
    m_LogicalPageSize.cy = MulDiv(m_LogicalPageSize.cx, 
                                  m_PaperSize.cy, m_PaperSize.cx);
#endif
...
void CGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    pInfo->m_rectDraw.right  = m_LogicalPageSize.cx 
                               - (LEFT_MARGIN+RIGHT_MARGIN) * m_CharSize.cx;
    ...
    while (nCurrPrintRow < GetRowCount())
    {
        ...
        for (int col = 0; col < GetColumnCount(); col++)
        {
            ...
            // Modified by PRAS 固定列打印采用程序设定的背景色
            if (col < GetFixedColumnCount())
            {
                DrawFixedCell(pDC, nCurrPrintRow, col, rect);
            }
            else
            {
    ...
    pDC->SetWindowOrg(-LEFT_MARGIN * m_CharSize.cx, 
                      -m_LogicalPageSize.cy + FOOTER_HEIGHT*m_CharSize.cy);
...
void CGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    while (pParentWnd)
    {
        ...
        // can happen if it is a CView, CChildFrm has the title
...
void CGridCtrl::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
    ...
    m_PrinterTitleFont.DeleteObject();
    m_PrinterHeaderFont.DeleteObject();
}

// Modified by PRAS 创建打印字体
void CGridCtrl::CreatePrinterFont(CDC* pDC)
{
    int nFontSize = -16;
    ...
    // 设置Gap的大小
    CFont fontBuffer;
    fontBuffer.CreateFont(-8,0,0,0, FW_NORMAL, 0,0,0, DEFAULT_CHARSET,
                             OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS,
                             DEFAULT_QUALITY,
                             DEFAULT_PITCH | FF_DONTCARE, strFontName);
    pDC->SelectObject(&fontBuffer);
    m_GapCharSize = 
        pDC->GetTextExtent(
        _T("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSATUVWXYZ"),52);
    m_GapCharSize.cx /= 52;
    pDC->SelectObject(pOldFont);
    fontBuffer.DeleteObject();

    // 创建标题字体并记录其大小
    strFontName = "黑体";
    m_PrinterTitleFont.CreateFont(-20,0,0,0, FW_NORMAL, 0,0,0, DEFAULT_CHARSET,
                                  OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS, 
                                  DEFAULT_QUALITY,
                                  DEFAULT_PITCH | FF_DONTCARE, strFontName);
    pDC->SelectObject(&m_PrinterTitleFont);
    m_TitleCharSize = 
        pDC->GetTextExtent(
        _T("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSATUVWXYZ"),52);
    m_TitleCharSize.cx /= 52;
    pDC->SelectObject(pOldFont);

    // 创建页眉字体并记录其大小
    strFontName = "宋体";
    m_PrinterHeaderFont.CreateFont(-12,0,0,0, FW_NORMAL, 0,0,0, 
                                   DEFAULT_CHARSET,
                                   OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS,
                                   DEFAULT_QUALITY,
                                   DEFAULT_PITCH | FF_DONTCARE, strFontName);
    pDC->SelectObject(&m_PrinterHeaderFont);
    m_HeaderCharSize = 
        pDC->GetTextExtent(
        _T("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSATUVWXYZ"),52);
    m_HeaderCharSize.cx /= 52;
    pDC->SelectObject(pOldFont);
}
...
void CGridCtrl::OnFixedRowClick(CCellID& cell)
{
    ...
    // Modified by PRAS 在ListMode模式下，允许排序
    if (m_bListMode && m_bSortItems)
    {
        // Modified by PRAS 发送固定行单元点击的消息
        SendMessageToParent(cell.row, cell.col, GVN_FIXEDROWCLICK);
        if (!m_bSortOnClick) 
            return;
        ...
        {
            SortTextItems(m_SortColumn, m_bAscending);  // Default Sort
        }
        else
        {
            // Modified by PRAS 采用给定的排序方式
            SortItems(m_pfnCompare, m_SortColumn, m_bAscending);
        }
        Invalidate();
    } 
    else if (cell.col < GetFixedColumnCount()) 
...
CSize CGridCtrl::GetCellExtent(int nRow, int nCol, CDC* pDC)
{
    ...
    size += CSize(3*m_nMargin, 1*m_nMargin);
...
BOOL CGridCtrl::DrawFixedCell(CDC* pDC, int nRow, int nCol,
                              CRect rect, BOOL bEraseBk)
{
    if (!m_bAllowDraw) 
        return FALSE;

    // Modified by PRAS
    if (m_iGridStyle == GRID_FUCHSIN)
    {
        COLORREF bkclr;
        bkclr = COLOR_FUCHSIN_FIXED;
        SetItemBkColour(nRow, nCol, bkclr);
    }
    else if (m_iGridStyle == GRID_BLUE)
    {
        COLORREF bkclr;
        bkclr = COLOR_BLUE_FIXED;
        SetItemBkColour(nRow, nCol, bkclr);
    }
    else
    {
    ...
    if (bEraseBk
#if !defined(_WIN32_WCE_NO_PRINTING) && !defined(GRIDCONTROL_NO_PRINTING)
        //&& !pDC->IsPrinting() //Modified by PRAS  打印固定行列采用设定背景色
#endif
        )
    {
        CBrush brush((Item.crBkClr == CLR_DEFAULT)
                        ? GetFixedBkColor() : Item.crBkClr);
        pDC->FillRect(rect, &brush);
    }
    pDC->SetTextColor((Item.crFgClr == CLR_DEFAULT)
                        ? GetFixedTextColor() : Item.crFgClr);
    ...
    //Modified by PRAS 焦点所在的行列不做特殊处理
#if !defined(_WIN32_WCE_NO_PRINTING) && !defined(GRIDCONTROL_NO_PRINTING)
        && !pDC->IsPrinting()
#endif
        )
...
BOOL CGridCtrl::DrawCell(CDC* pDC, int nRow, int nCol, 
                         CRect rect, BOOL bEraseBk)
{
    if (!m_bAllowDraw) 
        return FALSE;

    // Modified by PRAS 不同风格采用不同颜色
    // LUO 20140521 去除最后一行空白
    //if(nCol < GetColumnCount() - 1)
    if(nCol < GetColumnCount())
    {
        if (m_iGridStyle == GRID_FUCHSIN)
        {
            ...
            if (nRow % 2 == 0)
            {
                bkclr = COLOR_FUCHSIN_EVEN;
            }
            else
            {
                bkclr = COLOR_FUCHSIN_ODD;
            }
            SetItemBkColour(nRow, nCol, bkclr);
        }
        else if (m_iGridStyle == GRID_BLUE)
        {
            ...
            if (nRow % 2 == 0)
            {
                bkclr = COLOR_BLUE_EVEN;
            }
            else
            {
                bkclr = COLOR_BLUE_ODD;
            }
            SetItemBkColour(nRow, nCol, bkclr);
        }
    }
    ...
    COLORREF TextBkClr = (Item.crBkClr == CLR_DEFAULT)
                            ? GetTextBkColor() : Item.crBkClr;
    COLORREF TextClr   = (Item.crFgClr == CLR_DEFAULT)
                            ? GetTextColor()   : Item.crFgClr;
...
void CGridCtrl::OnEditCell(int nRow, int nCol, UINT nChar)
{
    ...
    // Modified by PRAS ,这里获取Cell后未判断其是否符合要求并可编辑，
    // 这些判断在进入本函数之前已经完成
    CGridCell* pCell = GetCell(nRow, nCol);
    
    // Modified by PRAS 如果表格的属性是GVET_LIST，则编辑时生成List
    if (pCell->nEditType == GVET_LIST)
    {
        if ((Item.nFormat & CBS_DROPDOWNLIST) == CBS_DROPDOWNLIST) 
            dwStyle = CBS_DROPDOWNLIST;
        else dwStyle = 0;
        CreateInPlaceListControl(rect, dwStyle, IDC_INPLACE_CONTROL, 
...
// Modified by PRAS 生成List
void CGridCtrl::CreateInPlaceListControl(CRect& rect, DWORD dwStyle, UINT nID,
                                         int nRow, int nCol,
                                         LPCTSTR szText, int nChar)
{
     new CInPlaceList(this, rect, dwStyle, nID, nRow, nCol, szText, nChar);
}
                                         ...
                                         UINT nEditType)
{
    ...
    switch (nEditType)  // Modified by PRAS 根据nEditType生成相应的edit
    {
    case GVET_INT:
        new CInPlaceIntEdit(this, rect, dwStyle, nID, nRow, nCol, 
                            szText, nChar);
        break;
    case GVET_DOUBLE:
        new CInPlaceDoubleEdit(this, rect, dwStyle, nID, nRow, nCol, 
                            szText, nChar);
        break;
    default:
...
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
    ...
    // Modified by PRAS 修改数据为空不改变原始数据
    if ((strCurrent != str) && (str != ""))
    {
...
void CGridCtrl::PrintPreview(const int iGridID)
{
    if (m_bPrintPreview)
    {
                                           ...
                                           LPARAM lParamSort)
{
    ...
    return _tcscmp(pCell1->szText, pCell2->szText);
}

// Modified by PRAS 数值比较方式
// 参考2.26版的函数实现
                                              ...
                                              LPARAM lParamSort)
{
    ...
    double dValue1 = atof((char*)pCell1->szText.GetBuffer(pCell1->szText.GetLength()));
    double dValue2 = atof((char*)pCell2->szText.GetBuffer(pCell2->szText.GetLength()));
...
// Modified by PRAS 设置单元格的编辑类型
void CGridCtrl::SetCellEditType(int nRow, int nCol, UINT nEditType)
{
    CGridCell* pCell = GetCell(nRow, nCol);
    pCell->nEditType = nEditType;
}



// Modified by PRAS 非类的成员
